...
import { CATEGORIES, DESTINATION_TAGS } from '@/lib/constants'
...
export const SectionDestinations: React.FC<Props> = ({ parentSlug = '' }) => {
  ...
  return (
    <section className="z-0 w-full max-w-screen-2xl mx-auto px-2 mb-4 lg:mb-8 xl:pb-10 mt-4 ">
      {/* Title & Filters */}
      <div className=" sticky top-20 md:top-[5.8rem] lg:top-24 bg-quaternary z-30 w-full h-full md:px-2 py-2 md:py-3 flex flex-col lg:flex-row items-start gap-2 lg:items-center justify-between ">
        <div className="w-full flex items-center justify-between">
          <h2 className=" flex items-center gap-2  ">
            <span className=" font-sign-painter text-4xl xl:text-5xl font-light text-neutral-600/70">
              {dictionary['Discover']}
            </span>{' '}
            <span className=" lg:mb-2 italic font-normal font-oswald text-3xl lg:text-4xl xl:text-5xl text-white">
            ...
            <TooltipProvider delayDuration={100}>
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    <SlidersHorizontalIcon className="w-6 h-6 opacity-60 group-hover:opacity-100 transition-all duration-200 ease-in-out " />
                  </button>
                </TooltipTrigger>
                <TooltipContent
                  sideOffset={6}
              ...
              <button
                ...
                className={` px-4 md:px-4 lg:px-6 pb-0.5 font-oswald text-base xl:text-xl text-nowrap lowercase border-2 border-white rounded-full transition-all duration-200 ease-in-out ${
                  tagsSelected.includes(tag.id)
                    ...
                    : 'bg-transparent text-neutral-600 hover:bg-white/40 hover:text-neutral-500'
                }`}
