...
export default function SectionDestinos({
  ...
  return (
    ...
    >
          ...
          <div className=" relative w-full h-full px-4 lg:px-6 2xl:px-2  py-4 pt-10 md:py-10 flex flex-col items-start justify-center gap-6 xl:max-w-screen-2xl mx-auto">
            ...
            <div className=" md:w-5/6 lg:max-w-[550px] bg-black/60 p-2 rounded-xl">
              <p className=" line-clamp-4 md:line-clamp-none text-sm md:text-lg xl:text-xl text-left text-pretty font-poppins font-extralight text-white ">
                ...
                , y a dejarte maravillar por la diversidad de culturas y
                paisajes que esperan ser explorados.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className=" relative z-0 w-full h-fit -my-4 md:-my-6 lg:-my-8">
        {items.length > 0 && (
          <div className="relative z-10 w-full px-2 md:px-4 lg:px-8 xl:px-4 2xl:px-2 md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-2xl mx-auto xl:h-1/3 grid grid-cols-2 gap-2 gap-y-4 md:grid-cols-2 lg:grid-cols-3 md:gap-4 lg:gap-10">
            {items.slice(0, 3).map((item, key) => (
              <HorizontalCard
                index={key}
                key={key}
                item={item}
                categorySlug="por-el-mundo/destinos"
              />
            ))}
          </div>
        )}
